---
title: Bafang M410
type: component
category: motors
parent: "[[Bafang]]"
domain: "[[Motors MOC]]"
tags:
  - motor
  - mid-drive
  - bafang
  - cargo-bike
  - ebike
generated_by: cargo-bikes-cli
motor_type: Mid-drive (crank motor)
nominal_power_W: 250
peak_power_W: 500
max_torque_Nm: 80
weight_kg: 3.5
bottom_bracket: 68/73 mm BSA threaded
voltage_V: 36
assist_type: Cadence + torque sensing
max_speed_kmh: 25
gear_sensor_input: true
---

## Bafang M410

## Overview

The Bafang M410 is a mid-drive crank motor from [[Bafang]]'s M-series lineup, positioned as an accessible, cargo-capable drive unit suitable for longtail and utility e-bikes. It sits below the flagship [[Bafang M620]] (Ultra) in Bafang's hierarchy, offering a lighter and more compact package while still delivering meaningful torque for loaded riding.

## Specs

| Spec                 | Value                    |
| -------------------- | ------------------------ |
| Motor Type           | Mid-drive (crank motor)  |
| Nominal Power        | 250 W                    |
| Peak Power           | ~500 W                   |
| Max Torque           | 80 Nm                    |
| Voltage              | 36 V                     |
| Weight               | ~3.5 kg                  |
| Bottom Bracket Shell | 68 / 73 mm BSA threaded  |
| Assist Type          | Cadence + torque sensing |
| Speed Limit (EU)     | 25 km/h                  |
| Gear Sensor Input    | Yes                      |

> [!warning] Spec Verification
> Specs above are based on pre-training knowledge. Verify against the [official Bafang product page](https://www.bafang-e.com) before publishing, as M-series revisions are released periodically.

## Bikes Using This Motor

- [[Oklö EVO Longtail]]

## Market Position

The M410 occupies Bafang's mid-range cargo segment — more refined than entry-level hub motors, but more cost-accessible than the [[Bafang M620]]. Its 80 Nm torque output is sufficient for moderate cargo loads and urban hilly terrain, making it a practical choice for longtail bikes targeting everyday utility riders rather than heavy-duty cargo haulers.

## Comparisons

| Motor                            | Torque | Weight  | Tier |
| -------------------------------- | ------ | ------- | ---- |
| [[Bafang M410]]                  | 80 Nm  | ~3.5 kg | Mid  |
| [[Bafang M620]]                  | 160 Nm | ~4.0 kg | High |
| [[Bosch Performance Line Cargo]] | 85 Nm  | ~3.2 kg | Mid  |
| [[Shimano EP6]]                  | 85 Nm  | ~2.9 kg | Mid  |

## Notes

- Gear sensor support helps protect the drivetrain under load — important for cargo use.
- Compatible with Bafang's own display ecosystem (DPC-series displays).
- EU 250 W / 25 km/h variant is the primary version used on European-market cargo bikes.

## Related

- [[Bafang]]
- [[Bafang M620]]
- [[Oklö EVO Longtail]]
- [[Motors MOC]]
