---
title: Monkey Bars Tote
type: accessory
category: footrests
manufacturer: "[[Yuba]]"
accessory_type: brand-specific
price_amount: 39
price_currency: EUR
compatible_bikes:
  - "[[Kombi]]"
  - "[[mini Boda Boda]]"
  - "[[Supercargo CL]]"
tags:
  - accessory
  - cargo-bike
  - yuba
  - brand-specific
  - basket
  - tote
  - footrests
generated_by: cargo-bikes-cli
---

# Monkey Bars Tote

The Monkey Bars Tote is a brand-specific accessory designed by [[Yuba]] for use with select models in their cargo bike lineup. It functions as a quick basket converter, allowing riders to instantly transform their Monkey Bars setup into a functional carrying basket — making it a fast and practical add-on for everyday hauling needs.

## Description

The Monkey Bars Tote is a quick-attach tote/basket designed specifically for Yuba's Monkey Bars accessory system. It converts the Monkey Bars into a basket in seconds, adding convenient cargo capacity without tools or complicated installation. Ideal for grocery runs, errands, or carrying loose items alongside passengers.

## Features

- Quick-attach basket converter for the Yuba Monkey Bars system
- Tool-free, fast installation and removal
- Adds functional cargo capacity to the rear deck
- Lightweight and practical for everyday use
- Designed to integrate seamlessly with Yuba's existing accessory ecosystem

## Compatibility

This accessory is designed specifically for [[Yuba]] cargo bikes equipped with the Monkey Bars system. It is not a universal accessory and is intended exclusively for compatible Yuba models.

### Compatible Bikes

- [[Kombi]]
- [[mini Boda Boda]]
- [[Supercargo CL]]

## Pricing

| Region | Price |
|--------|-------|
| Europe | €39   |

## Notes

As a brand-specific accessory, the Monkey Bars Tote is sold directly through [[Yuba]] and authorised Yuba dealers. Verify compatibility with your specific model year before purchasing.
