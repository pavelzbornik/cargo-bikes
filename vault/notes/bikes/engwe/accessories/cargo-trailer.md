---
title: "Cargo Trailer"
type: accessory
category: towing
accessory_type: brand-specific
manufacturer: "[[Engwe]]"
price_amount: "150–300"
price_currency: EUR
compatible_bikes:
  - "[[ENGWE LE20 Cargo]]"
tags:
  - accessory
  - towing
  - cargo-trailer
  - engwe
  - brand-specific
generated_by: cargo-bikes-cli
---

# Cargo Trailer

A brand-specific cargo trailer designed for use with the [[ENGWE LE20 Cargo]] electric cargo bike. It extends the load-carrying capacity beyond what the bike frame and integrated rack can accommodate, allowing riders to haul bulkier or heavier goods without overloading the bike itself.

## Description

The Engwe Cargo Trailer attaches directly to the [[ENGWE LE20 Cargo]] and provides an additional towed platform for oversized or heavy cargo. It is designed and sold by [[Engwe]] as a dedicated accessory for their cargo bike line, ensuring a compatible hitch connection with the LE20 frame geometry.

## Features

- Extended load capacity beyond the bike's built-in carrying limits
- Dedicated hitch coupling matched to the [[ENGWE LE20 Cargo]] rear frame
- Sturdy cargo platform suitable for boxes, crates, or loose loads
- Low-profile design to maintain stability when loaded
- Designed specifically for use with Engwe cargo bikes

## Compatibility

This trailer is a brand-specific accessory made by [[Engwe]] for their own bikes. It is designed to pair with the [[ENGWE LE20 Cargo]] and may not be compatible with other brands or frame types without modification.

| Field           | Detail                      |
| --------------- | --------------------------- |
| Compatible bike | [[ENGWE LE20 Cargo]]        |
| Attachment type | Rear hitch (brand-specific) |
| Accessory type  | Brand-specific              |
| Manufacturer    | [[Engwe]]                   |

## Pricing

| Region | Price Range |
| ------ | ----------- |
| Europe | €150 – €300 |

> Prices may vary by retailer and market. Check the official Engwe website or authorised dealers for current pricing.
