---
title: Fatstreet Two-Seater Luggage Rack
type: accessory
category: racks
manufacturer: "[[Unikride]]"
price_amount: 99
price_currency: EUR
accessory_type: brand-specific
compatible_bikes:
  - "[[Fatstreet Classic - Two-seater electric cargo bike (Female)]]"
tags:
  - accessory
  - rack
  - cargo
  - brand-specific
  - unikride
  - fatstreet
generated_by: cargo-bikes-cli
---

## Fatstreet Two-Seater Luggage Rack

The Fatstreet Two-Seater Luggage Rack is a brand-specific rear rack designed and manufactured by Unikride exclusively for the Fatstreet Classic two-seater electric cargo bike range. It extends the bike's carrying capability by adding a dedicated rear cargo platform, making it well suited for daily errands, grocery runs, or general load hauling.

## Overview

Designed specifically for the Unikride Fatstreet platform, this rack integrates directly with the frame geometry of the Fatstreet Classic, ensuring a secure and stable fit without the need for adapters or modifications. As a factory-matched accessory, it preserves the bike's handling characteristics even when loaded.

**Price range:** €99 – €149

## Features

- Rear-mounted cargo platform purpose-built for the Fatstreet Classic frame
- Expands total carrying capacity beyond the built-in two-seater configuration
- Compatible with standard panniers, cargo bags, and bungee net systems
- Sturdy construction to handle everyday loads
- Integrates cleanly with the Fatstreet Classic's existing mounting points
- Designed to complement the bike's electric drivetrain without interfering with battery or motor components

## Compatibility

This luggage rack is designed specifically for use with:

- [[Fatstreet Classic - Two-seater electric cargo bike (Female)]]

As a brand-specific accessory made by [[Unikride]], it is engineered to match the Fatstreet Classic frame geometry precisely. It is not intended as a universal rack and may not be compatible with other cargo bike brands or models without modification.

## Notes

- Verify compatibility with your specific Fatstreet Classic frame variant before purchasing
- Check with [[Unikride]] or an authorised dealer for maximum load rating and installation guidelines
- Pricing may vary by region and retailer; confirm current pricing at point of sale
